...
function configure(pkg, env, target) {
  const inputName = pkg.name.split("/")[1]
  console.log(env, target, inputName)
  const isProd = env === "production"
  const isUmd = target === "umd"
  const isModule = target === "module"
  const isCommonJs = target === "cjs"
  const input = `packages/${inputName}/src/index.ts`
  const deps = []
  ...
  const plugins = [
    ...
    typescript({
      abortOnError: false,
      tsconfig: `./packages/${inputName}/tsconfig.json`,
    ...
    // modules by default.
    commonjs({
      exclude: [`packages/${inputName}/src/**`],
    ...
    babel({
      runtimeHelpers: true,
      include: [`packages/${inputName}/src/**`],
  ...
  if (isUmd) {
    return {
      ...
      output: {
        format: "umd",
        file: `packages/${inputName}/${isProd ? pkg.umdMin : pkg.umd}`,
        exports: "named",
        name: startCase(inputName).replace(/ /g, ""),
  ...
  if (isCommonJs) {
    return {
      ...
      output: [
        {
          file: `packages/${inputName}/${pkg.main}`,
          format: "cjs",
          exports: "named",
          sourcemap: true
        }
  ...
  if (isModule) {
    return {
      ...
      output: [
        {
          file: `packages/${inputName}/${pkg.module}`,
          format: "es",
          sourcemap: true
        }
      ],
